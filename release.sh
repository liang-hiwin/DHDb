...
function AnalyseData() {
    if [ ! -f "../dhdb_dead.txt" ]; then
        ...
        if [ ! -f "../dhdb_alive.txt" ]; then
            dhdb_data=($(cat ../dhdb_dead.txt > ./dhdb_data.old && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dhdb_data.old ./dhdb_data.tmp | sort | uniq | head -n 1000 | awk "{ print $2 }"))
        else
            dhdb_data=($(cat ../dhdb_alive.txt ../dhdb_dead.txt > ./dhdb_data.old && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dhdb_data.old ./dhdb_data.tmp | sort | uniq | head -n 1000 | awk "{ print $2 }"))
            if [ "${#dhdb_data[@]}" == 0 ]; then
                dhdb_data=($(get_total_line=$(sed -n '$=' ../dhdb_alive.txt) && for (( tmp = 0; tmp < 800; tmp++ )); do generate_radom_line=$(( RANDOM%${get_total_line} )); generate_radom_line=$[generate_radom_line + 1]; sed -n "$generate_radom_line"p ../dhdb_alive.txt; done > ./dhdb_alive.old && get_total_line=$(sed -n '$=' ../dhdb_dead.txt) && for (( tmp = 0; tmp < 200; tmp++ )); do generate_radom_line=$(( RANDOM%${get_total_line} )); generate_radom_line=$[generate_radom_line + 1]; sed -n "$generate_radom_line"p ../dhdb_dead.txt; done > ./dhdb_dead.old && cat ./dhdb_alive.old ./dhdb_dead.old | sort | uniq | awk "{ print $2 }"))
            fi
        fi
    fi
}
